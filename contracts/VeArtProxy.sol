...
contract VeArtProxy is IVeArtProxy {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "background_color": "121a26", "description": "Magnetar Finance is a next-generation AMM that combines the best of Curve, Solidly and Uniswap, designed to serve as the cross-chain/multi-chain liquidity hub. Magnetar NFTs vote on token emissions and receive bribes and fees generated by the protocol.", "image": "data:image/svg+xml;base64,',
    ...
    /// @dev Calculates the number of digits before the "decimal point" in an NFT's veMGN balance.
    ///      Input expressed in 1e18 format.
    function numBalanceDigits(uint256 _balanceOf) internal pure returns (int256 digitCount) {
        uint256 convertedveMGNvalue = _balanceOf / 1e18;
        while (convertedveMGNvalue != 0) {
            convertedveMGNvalue /= 10;
            digitCount++;
        }
    }
    ...
    /// @dev Determines the number of lines in the SVG. NFTs with less than 10 veMGN balance have zero lines.
